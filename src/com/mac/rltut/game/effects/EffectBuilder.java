...
public class EffectBuilder {
    ...
    public static Effect randomWeaponEffect(int z, Random random){
        Pool<Effect> pool = new Pool<Effect>(random);
        pool.add(new Poison(1 + ((z / 2) + 1), 5 + ((z / 3) * 2), 0.75f), 100);
        pool.add(new Blind(10 + (z / 4) * 2, 0.25f), 50);
        pool.add(new Freeze(10 + (z / 4) * 2, 0.25f), 20);
        pool.add(new Burn(1, 10 + (z / 4) * 2, 0.25f), 30);
        return pool.get();
    }
